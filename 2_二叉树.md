...
public:
    ...
    int oneSideMax(TreeNode* root) { // 单边的最大路径和(贡献值)：返回从当前节点向下走的最大路径和（只能选一边）
        if (!root) return 0;
        int left = max(0, oneSideMax(root->left));    // 负数贡献直接舍弃
    ...
    void dfs(TreeNode* root) { // 作用：遍历每个节点，计算以当前节点为顶点的路径和
        ...
        // 计算：左最大 + 当前节点 + 右最大
        int L = max(0, oneSideMax(root->left));
