...
public:
    ...
    void flatten(TreeNode* root) {
        ...
        // 连接成链表
        TreeNode* pre = allNode[0];
        for (int i=1; i<allNode.size(); i++) {
            TreeNode* cur = allNode[i];
            pre->right = cur;
            pre->left = NULL;
            //
            pre = cur;
...
class Solution {
public:
    int rightSum = 0; // 右子树的和(包含root) 

    void dfs(TreeNode* root) {
        if(!root) return;
        dfs(root->right); // 先遍历 右子树
        {
            rightSum += root->val;
            root->val = rightSum; // 更新: root的值 = 右子树的和(包含root)
        }
        dfs(root->left);  // 后遍历 左子树
    }
...
public:
    TreeNode* dfs(TreeNode* root1, TreeNode* root2) {
        if (!root1 && !root2) { // x x
            ...
            TreeNode* newNode = new TreeNode(root1->val + root2->val); // val=合并两个数的值
            newNode->left = dfs(root1->left, root2->left);             // 递归合并左子树
            newNode->right = dfs(root1->right, root2->right);          // 递归合并右子树
            return newNode;
        } else if (root1) { // √ x
            return root1;
        } else { // x √
...
1. 小问题: count = 节点总数
2. 大问题: root的坡度 = abs(count(左) - count(右))
...
2.2 任意节点 ---> 所有孩子节点
	    测试用例:[1,2,3,null,5]
	    测试结果:["1","1->2","1->2->5","1->3","2","2->5","5","3"]
    vector<vector<int>> allPath;
    void dfs(TreeNode* root, vector<int> path) {
        if (!root) return;
...
给定一个二叉树，返回所有从【根节点到叶子节点】的路径。

- 扩展题目 8.1.2 [129] 求根节点到叶节点数字之和
...
public:
    ...
    void dfs(TreeNode* root, int targetSum) {
        ...
        if (!root->left && !root->right && root->val == targetSum) { // 叶子
            ans = true;
            return;
        }
...
给你二叉树的根节点 root 和一个整数目标和 targetSum ，找出所有 从`根节点到叶子节点` 路径总和等于给定目标和的路径。
...
路径 `不需要从【根节点开始】，也不需要【在叶子节点结束】`，但是路径方向必须是向下的（只能从父节点到子节点）。 
...
public:
    ...
    void dfs(TreeNode* root, /*vector<int> path,*/ long long targetSum) { // 以root为根节点的所有路径的总和为targetSum
        if (!root) return;

        // path.push_back(root->val);

        if (root->val == targetSum) { // 找到一个路径
            ans++;
        }
    ...
    void searchAll(TreeNode* root, int targetSum) {
        if (!root) return;

        dfs(root, /*{},*/ targetSum);     // 求root为根的目标总和=targetSum的总路径树
        searchAll(root->left, targetSum); // 求root->left
        searchAll(root->right, targetSum);// 求root->right
    }

    int pathSum(TreeNode* root, int targetSum) {
        searchAll(root, targetSum);
        return ans;
    }
};
...
`不需要从【根节点开始】，也不需要【在叶子节点结束】`

- 答案 和 ‘后序遍历’ 一样


2. 大问题: root->val + 小问题(left) + 小问题(right)

1. 小问题: oneSideMax = 单边的最大路径和(贡献值)
2. 大问题: root的最大路径和 = root->val + (0, oneSideMax(left)) + (0, oneSideMax(right)) 
...
public:
    ...
    int oneSideMax(TreeNode* root) { // 作用：返回从当前节点向下走的最大路径和（只能选一边）
        if (!root) return 0;
        int L = max(0, oneSideMax(root->left));    // 负数贡献直接舍弃
        int R = max(0, oneSideMax(root->right));
        return root->val + max(L, R);
    }
    void dfs(TreeNode* root) { // 作用：遍历每个节点，计算以当前节点为顶点的路径和
        if (!root) return;
        dfs(root->left);   // 遍历左子树
        dfs(root->right);  // 遍历右子树

        ans = max(ans,
			      root->val + max(0, oneSideMax(root->left)) + max(0, oneSideMax(root->right))
			);
    }
...
public:
    ...
    int onSideMax(TreeNode* root) {
        if (!root) return 0;
        int L = max(0, onSideMax(root->left));
        int R = max(0, onSideMax(root->right));
