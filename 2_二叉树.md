...
public:
    // 全局答案，记录目前见过的最大路径和
    // 初始化为 INT_MIN，是为了处理全是负数的情况
    int ans = INT_MIN;

    // dfs(root) 返回：
    // 从当前节点 root 出发，向下走到某个子节点时，
    // 能得到的最大“单边路径和”
    //
    // 注意：返回给父节点时，只能选择左边或右边其中一条路径，
    // 不能同时走左右两边，否则路径会在父节点处产生分叉
    int dfs(TreeNode* root) {
        ...
        // 这里是在计算“经过当前节点的完整路径”，可以左右都要。
        // 以当前 root 作为路径最高点时，
        // 最大路径可以是：左边路径 + root + 右边路径
        //
        // 这里可以同时使用 L 和 R，
        // 因为这是在更新最终答案，不是返回给父节点
        ans = max(ans, root->val + L + R);

        // 这里是返回给父节点的“单边路径”，只能选一边。
        // 返回给父节点时，当前节点只能接一条边往上连
        // 所以只能选择：
        // root + 左路径
        // root + 右路径
        // 或者如果这两者都是负贡献，就返回 0，表示父节点不选当前分支
        return max(max(root->val + L, root->val + R), 0);
    }
...
public:
    ...
    // 从当前节点 root 出发，继续往下找路径
    // sum 表示还差多少可以凑成 targetSum
    void dfsFrom(TreeNode* root, long long sum) {
        if (!root) return;
        ...
        // 继续向左、向右延伸路径
        dfsFrom(root->left, sum - root->val);
        dfsFrom(root->right, sum - root->val);
    }
